...
<div class="row" ng-controller="MainCtrl" ng-init="getDataHdr(); getDataLn();">
  <div class="col-md-12 col-xs-12">
    ...
    <div class="row x_title">
           ...
           <h2><a href="<?=site_url();?>/partpurchase_ctrl/po_sv_manage"><i class="fa fa-credit-card"></i>&nbsp;รายการสั่งซื้ออะไหล่ Service Center&nbsp;&nbsp;&nbsp;</a>: <?=$po_num;?></h2>
          <ul class="nav navbar-right panel_toolbox">
            <li><a href="<?=site_url();?>/partpurchase_ctrl/po_sv_manage"><i class="glyphicon glyphicon-backward"></i>&nbsp;กลับไป</a></li>
          </ul>
    <div class="clearfix"></div>
    </div>
...
<script>
  ...
  AppJS.controller('MainCtrl', ['$scope','$http','$uibModal', '$log', '$document','$timeout', function ($scope,$http,$uibModal, $log, $document,$timeout)
  {
      ...
      $scope.getDataHdr = function(){
              ...
              $http.get(site+"/partpurchase_ctrl/getHdrListPurchaseSV/"+po)
      ...
      $http.get(site+"/partpurchase_ctrl/getLnListPurchaseSV/"+po)
      .then(function(response) 
        {
      ...
      $scope.incrementList = function(data){
        ...
        if(qty <= $scope.limitOrder){
            ...
            var request = $http({
                method: "post",
                url: site+"/partpurchase_ctrl/updateQtyLineOrderSV",
                data: {
      ...
      $scope.deleteOrderList = function(data){
              ...
              var request = $http({
                  method: "post",
                  url: site+"/partpurchase_ctrl/deleteOrderLineSV",
                  data: {
                          id : data.partorderline_id,  
...
angular.module('AppJS').controller('ModalInstanceCtrl', function ($scope, $uibModalInstance, items, $http) {
  ...
  $scope.doUpdate = function () {
      ...
      var request = $http({
      method: "post",
      url: site+"/partpurchase_ctrl/updateQtyCancelOrderSV",
      data: {
